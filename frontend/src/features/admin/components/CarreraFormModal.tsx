...
import type { Carrera, CarreraFormData } from '@/features/admin/types'
...
interface CarreraFormModalProps {
  item?: Carrera | null
  onSave: (data: CarreraFormData) => void
  onClose: () => void
  loading: boolean
}

export function CarreraFormModal({ item, onSave, onClose, loading }: CarreraFormModalProps) {
  const { register, handleSubmit, formState: { errors }, reset } = useForm<CarreraFormData>({
