import { useCarreras, useDeleteCarrera } from '@/features/admin/hooks/useEstructura'
import type { Carrera } from '@/features/admin/types'
...
export function CarrerasTable({ onEdit, onNew }: CarrerasTableProps) {
  const { data, isLoading, error } = useCarreras()
  const deleteMutation = useDeleteCarrera()

  if (isLoading) return <div className="flex justify-center py-8"><Spinner /></div>
  if (error) return <Alert variant="error" message="Error al cargar carreras" />
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <Button onClick={onNew}>Nueva Carrera</Button>
      </div>
          ...
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left font-medium text-gray-600">Creación</th>
