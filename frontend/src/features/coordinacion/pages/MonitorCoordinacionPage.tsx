...
import { DateRangePicker } from '@/features/coordinacion/components/DateRangePicker'
import { useMonitorCoordinacion } from '@/features/coordinacion/hooks/useMonitorCoordinacion'
import type { MonitorFiltrosCoordinacion } from '@/features/coordinacion/types'

export default function MonitorCoordinacionPage() {
  const [filtros, setFiltros] = useState<MonitorFiltrosCoordinacion>({})
  const { data: entries, isLoading, isFetching } = useMonitorCoordinacion(filtros)
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-gray-900">Monitor de Coordinación</h2>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <DateRangePicker
          ...
          onDesdeChange={(v) => updateFiltro('fecha_desde', v)}
          onHastaChange={(v) => updateFiltro('fecha_hasta', v)}
        />
        <select
          className="rounded-md border border-gray-300 px-3 py-1.5 text-sm"
