import { useSalariosBase, useDeleteSalarioBase } from '@/features/finanzas/hooks/useGrillaSalarial'
import type { SalarioBase } from '@/features/finanzas/types'
...
export function SalarioBaseTable({ onEdit, onNew }: SalarioBaseTableProps) {
  const { data, isLoading, error } = useSalariosBase()
  const deleteMutation = useDeleteSalarioBase()

  if (isLoading) return <div className="flex justify-center py-8"><Spinner /></div>
  if (error) return <Alert variant="error" message="Error al cargar salarios base" />
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <Button onClick={onNew}>Nuevo Salario Base</Button>
      </div>
